...
function mvap_sans_init () {
    ...
    if [ "$1" = "-f" ]; then # si c'est un fichier
        nom_fichier="$2"
        cat "$nom_fichier" | grun src/Grammar 'calcul' > fichier.mvap
    else        
        expression="$1"
        echo "$expression" > entree.temp

        cat entree.temp | grun src/Grammar 'calcul' > fichier.mvap
        rm entree.temp
    fi
...
function mvap_debug () {
    ...
    if [ "$1" = "-f" ]; then # si c'est un fichier
        ...
        cat "$nom_fichier" | grun src/Grammar 'calcul' -tokens
        cat "$nom_fichier" | grun src/Grammar 'calcul' > fichier.mvap
    else        
        expression="$1"
        echo "$expression" > entree.temp

        cat entree.temp | grun src/Grammar 'calcul' -tokens
        cat entree.temp | grun src/Grammar 'calcul' > fichier.mvap
