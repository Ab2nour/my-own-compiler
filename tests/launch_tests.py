...
def create_tests_list(filename):
    ...
    with open(f"tests/{filename}.xml") as fichier_test:
        ...
        for t in root.findall("test"):
            ...
            try:
                t_dict["entree"] = format_whitespace(
                    t.findall("entree")[0].text.strip()
                )
            ...
            try:
                t_dict["sortie"] = format_whitespace(
                    t.findall("sortie")[0].text.strip()
                )
    ...
    with open(NOM_FICHIER_SH, mode="a") as fichier_sortie:
        ...
        for test in tests:
            ...
            fichier_sortie.write(
                f"test_expr '{entree}' '{sortie}' '{stdin}' '{description}'\n"
            )
